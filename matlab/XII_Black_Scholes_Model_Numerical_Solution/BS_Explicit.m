function V = BS_Explicit(xspan,Nx,ic,bcl,bcr,tspan,Nt,r,v)
...
L1 =                 0.5*v^2*rho - (r-0.5*v^2)*(dt/(2*dx));
C1 = (1-r*dt) + (-2)*0.5*v^2*rho;
R1 =                 0.5*v^2*rho + (r-0.5*v^2)*(dt/(2*dx));
...
for j=2:Nt
    V(2:Nx-1,j) = L1*V(1:Nx-2,j-1)+C1*V(2:Nx-1,j-1)+R1*V(3:Nx,j-1); % Expli 
end
end